...
try {
  ...
  foreach ($check in @(
    ...
    @{ Path = 'extension\direct-network-diagnostics.js'; Pattern = 'details?.documentId === attempt.documentId' },
    @{ Path = 'extension\direct-network-diagnostics.js'; Pattern = 'onCompleted.addListener' },
    @{ Path = 'extension\service-worker.js'; Pattern = 'createDirectObservationContext(sender, chrome.runtime)' },
