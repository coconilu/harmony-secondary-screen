...
| 平板型号（只作证据） | 待填写 |
| Wi-Fi / AP / VPN 状态 | 待填写 |
| 源尺寸 / 协商合同 / 实际 H.264 配置 | 待填写 |

## HWC6 自动媒体合同门禁（待执行）

以下是 HWC6 新增门禁，不能由单元测试、HAP 构建或历史 HWC5 真机记录替代：

| 场景 | 操作 | 通过条件 |
| --- | --- | --- |
| 横屏高分辨率 | 发送 16:9 与 4:3 高分辨率页面 | 合同不放大、偶数对齐且不超过 1920×1080/2073600 像素；画面按真实比例 contain |
| 竖屏 | 发送竖屏尺寸页面 | 保持竖屏方向和比例，无拉伸或裁切 |
| 能力降级 | 在只接受 AVC Level 4.0 的设备/测试环境发送高分辨率源 | 自动降级到 ≤1280×720，无用户质量选择器 |
| 帧率 | 分别发送 24/30/50/60/>60 fps 来源 | 目标 `maxFps` ≤60，不复制、不插帧；监控区分目标与实测帧率 |
| 稳定尺寸变化 | 捕获中让来源尺寸稳定变化 | 新 `sourceEpoch`、新合同和解码器生效；首个 SPS/PPS + IDR 后恢复，无旧帧串入 |
| 尺寸抖动 | 制造瞬时变化与 A→B→A | 不触发不必要重配置 |
| 旧协议 | 用 HWC5 扩展或 Receiver 尝试连接 | 控制/二维码/binary 明确协议不匹配；已保存 credential 不因失败被删除 |
| 旧配对记录升级 | 两端保留 HWC5 时代应用数据后升级 HWC6 | 无需扫码即可鉴权播放；若应用被卸载或数据被清除则按正常流程重新配对 |

完成时记录 exact commit、扩展 `0.7.0`、真实协商宽高/maxFps、编码 profile/level、可见画面比例和
尺寸变化恢复结果。当前分支在真实 Edge + HarmonyOS 平板执行前不得把上述行标为 PASS。
...
| 短码安全分流 | 真实 UI 只证明默认 `.local` + 六位短码被拒绝并给出数字 IPv4 下一步；“零 token”结论仅引用 exact-head 自动测试，不能声称由截图或真实 UI 直接观测。 |
| 连接耗时 | 默认 `.local` + 新二维码的界面完成速度仅有用户“几乎立即、人工估计小于 0.1 秒”的主观记录；该估计低于手工计时的可信分辨率，不作为性能指标或时延门禁。 |
| 后续网络场景 | 真实 Wi-Fi 地址变化与真实同名 mDNS 冲突均未执行，转交后续 Issue #22，不作为 Issue #19 / PR #20 的关闭门禁；本节约 10 秒的 Wi-Fi 断开只证明短时连接恢复，不代表地址发生变化。 |
| 隐私 | 本节及仓库外脱敏监控均未记录实际 IP、token、短码、credential、事件时间戳、网页 URL/标题、视频帧、设备标识符或签名路径/秘密。 |
...
| 协议 | 当时 HWC5 无变更，恢复沿用 `keyframe` + `requireCodecConfig: true`；仅属历史记录 |
