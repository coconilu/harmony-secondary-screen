...
| 通道 | 传输 | 默认端口 | 绑定与用途 |
| --- | --- | --- | --- |
| Control | TCP | 47100 | Host 只绑定用户在应用中确认的物理 Wi-Fi IPv4；配对、心跳、恢复、触控、遥测 |
| Video | UDP | 47101 | Receiver 本地收包；Host 只向已配对 TCP 对端 IP 发送 H.264 Annex-B |

未经用户确认的 Wi-Fi profile、以太网/VPN 虚拟接口、`0.0.0.0` 控制监听和公网端口映射都不属于
v0.1。控制端同时只接受一个 Receiver。网络授权只记录应用允许的 NLM profile ID，不修改 Windows
的公用/专用分类、VPN 或其他网络设置。
...
`requireCodecConfig=true` 表示 Receiver 在丢包、会话开始或 AVCodec `Flush` 后，不接受普通增量帧；Host 下一帧必须同时携带
SPS/PPS 与 IDR，并设置 `keyframe`、`codec-config` 标志。Receiver 执行 `Flush` 后必须重新 `Start`；若任一步失败则重建解码器，
在收到同时带有 codec config 的关键帧前持续丢弃增量帧。
...
- 控制监听只绑定用户明确确认过的物理 Wi-Fi；以太网和 VPN 虚拟网卡不参与候选；
- 交互式启动或服务安装必须展示当前 Wi-Fi 名称与风险说明；用户拒绝时不监听；
- Host 不调用 `INetwork::SetCategory`，VPN 可以保持连接；Windows 的公用/专用分类不参与应用授权；
- Host 在 bind、accept 以及已连接会话循环中重新查询 Network List Manager；已确认的 Wi-Fi 断开、
  profile 切换或 IPv4 变化时先撤销数据面 session epoch，保证撤销后不再发送 UDP 分片，再发送
  `{"type":"error","code":"wifi_not_allowed"}` 并关闭控制连接；
