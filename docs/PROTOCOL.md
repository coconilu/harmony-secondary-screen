...
| 通道 | 传输 | 默认端口 | 绑定与用途 |
| --- | --- | --- | --- |
| Control | TCP | 47100 | Host 只绑定用户确认且 Windows 分类为“专用”的物理 Wi-Fi IPv4；配对、心跳、恢复、触控、遥测 |
| Video | UDP | 47101 | Receiver 本地收包；Host 只向已配对 TCP 对端 IP 发送 H.264 Annex-B |

未经用户确认的公共网络接口、以太网/VPN 虚拟接口、`0.0.0.0` 控制监听和公网端口映射都不属于
v0.1。控制端同时只接受一个 Receiver。网络准备流程仅改变当前物理 Wi-Fi 的 Windows 配置类别，
不改变任何控制帧或视频包格式。
...
- 当前家庭 Wi-Fi 被 Windows 标记为“公用”时，交互式启动或服务安装先说明风险，再经 UAC 只把该
  Wi-Fi 配置文件改为“专用”；拒绝确认或授权时继续拒绝监听；
- 如果 Windows 把物理 Wi-Fi 与 VPN、以太网或未知连接聚合为同一网络对象，Host 拒绝自动修改其类别；
- Host 在 bind、accept 以及已连接会话循环中重新查询 Network List Manager；网络从“专用”切换为
  “公共”时先撤销数据面 session epoch，保证撤销后不再发送 UDP 分片，再发送
  `{"type":"error","code":"network_not_private"}` 并关闭控制连接；
