...
家庭 Wi-Fi 若被 Windows 标记为“公用”，Host 会先展示可信网络说明；用户确认后通过 UAC 只把当前
Wi-Fi 标记为“专用”，无需手动打开系统设置。用户拒绝时不会监听，也不会回退到 `0.0.0.0`；运行中
切换为“公用”会先撤销 session 数据面并停止后续 UDP，再关闭 listener 和现有控制连接。服务安装器
同时创建仅适用于“专用 + 无线 + 本地子网”的 TCP 47100 入站规则，卸载时一并删除；不会为公用网络、
VPN/以太网或 UDP 创建放行规则。

极少数情况下，Windows 会把 Wi-Fi 与 VPN/以太网聚合到同一个 `INetwork`。此时修改类别会连带影响
整个聚合网络，Host 会拒绝自动修改并要求暂时断开被合并的连接；Wi-Fi 完成一次信任后即可重新连接 VPN。
...
```powershell
# 管理员 PowerShell；若当前 Wi-Fi 为公用网络，会先明确询问是否信任
