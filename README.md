...
`out/harmony-build-tools/`。Hvigor、npm 和 ohpm 的构建缓存也隔离在该目录，不会修改 DevEco
安装目录或用户全局 cache；首次构建需要访问 HarmonyOS 官方 npm 仓库，后续复用项目隔离工具链。
Release 构建产物是
`receiver/entry/build/default/outputs/default/entry-default-unsigned.hap`。
